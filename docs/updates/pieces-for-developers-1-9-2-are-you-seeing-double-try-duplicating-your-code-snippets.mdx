---
title: Pieces for Developers 1.9.2 | Are you seeing double? ✌️
description: Hope everyone is excited to try out our new snippet duplication feature! As developers, we often copy and paste code and then make edits in-line. But it’s helpful to have the original version in case you need it later.
---

<link rel="canonical" href="https://code.pieces.app/updates/pieces-for-developers-1-9-2-are-you-seeing-double-try-duplicating-your-code-snippets" />

export const Tag = props => {
    return <div style={{ padding: "2px 16px", borderRadius: "20px", border: "1px solid gray", maxWidth: "fit-content" }}>
        { props.children }
    </div>
};

export const GridFlexRow = props => {
    return <div style={{
        display: 'flex',
        flexDirection: 'row',
        justifyContent: 'start',
        flexWrap: "nowrap"
    }}>{props.children}</div>;
};

![](https://uploads-ssl.webflow.com/6143afec68f555387049efb3/63405a907f56de239726d799_PFD1_9_2_a0bbd919fd400d0ca511983e39100cb2_2000.jpeg)
<GridFlexRow><Tag>bug fixes</Tag> <Tag>new features</Tag></GridFlexRow>

# Pieces for Developers 1.9.2: Are you seeing double? ✌️
> **Published by the Pieces Team** | May 1st, 2023

Hope everyone is excited to try out our new snippet duplication feature! As developers, we often copy and paste code and then make edits in-line. But it’s helpful to have the original version in case you need it later.

With snippet duplication, you can press ```⌘ + D``` or open the quick menu and select “Duplicate Snippet” when looking at a code snippet in your Pieces for Developers window, and it will duplicate that snippet so you have a new version that you can work on.

Check it out in action here:
![](https://uploads-ssl.webflow.com/6143afec68f555387049efb3/6345ad5b03255e892da27152_ezgif_com-gif-maker_3896c72d0613d4eb36929529b659d3ce.gif)

## Auto-Update Sharable Links 🔗
With today’s release, shareable links will update on their own without having to do “update link”  manually. Changed the title? Updated. Created a custom domain so your links have your name in them? Updated and your links now look polished without having to manually update every shared snippet you sent in the past.

The best part is when you change data that is on a shareable link inside the code snippet itself, that will update too. So anyone who uses a link that was created in the past can stay up to date with any changes made to a snippet in the future.

To enable auto-updating, first create a shareable link, then change data to get the notification. Here is an example of what that popup looks like:

![](https://uploads-ssl.webflow.com/6143afec68f555387049efb3/6345ad5b03255e9fb3a27151_updatesharablelink_bcfe6b373b97e7876128b453fd6fe4e3_800.png)

If you click “Don’t ask me again” - Pieces will auto-update in the background whenever your snippet changes.

## Bugs, anyone? 🐛
More bugs squashed, which means a smoother experience for you as these new features come out. 

Some of you reported issues with opening shared snippets in the browser, so we upgraded a lot of small things around the sharing experience. Some other small patches include the following:
- Updates to issues with flashing screens during onboarding
- Improvements to the shareable link logic for smoother transitions from the browser into Pieces
- More clickable area added to the “suggested tag” section of the tags menu

## Try new features before everyone else
Sign up for the beta program to get access to new features before they’re deployed to all users. You’ll help us catch bugs and provide useful feedback to make Pieces work for you and others.  <a href="https://getpieces.typeform.com/to/XGGlUqEI" target="_blank" rel="noopener noreferrer">Sign up here for the beta program</a>.

## Support
As always, if you run into issues or have feedback, please fill out <a href="https://getpieces.typeform.com/to/mCjBSIjF" target="_blank" rel="noopener noreferrer">this quick form</a> or email us at <a href="mailto:support@pieces.app" target="_blank" rel="noopener noreferrer">support@pieces.app</a> and we’ll be in touch as soon as possible!
