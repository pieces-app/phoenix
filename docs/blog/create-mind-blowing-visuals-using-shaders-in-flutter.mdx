...
description: Most of the time, these rich visuals are created using a combination of Flutter shaders. Take a look at the visuals in this article, where shaders play a critical role. Today, you’ll learn about a specific type of shader called a Fragment Shader and how to use it in your Flutter application. Outside of specifically discussing Flutter Fragment Shaders, I’ll touch on many other things including giving you some basic background on shaders for those who may not be as familiar.
---
...
- GLSL has built-in variables like <em gl_FragCoord and <em gl_FragColor. <em gl_FragCoord gives you the pixel vector position, and you set the pixel color through the <em gl_FragColor. When you specify a vec4, you’re actually setting the rgba values of the color for that pixel.
- GLSL allows us to create conditional compilation blocks and defining values in the pre-compilation stage. These blocks or values start with <em #. <em #ifdef(if defined)#endif is a conditional operation block that is checked before compilation. The 2nd line executes if the GL_ES is defined, which is a version of GLSL for embedded systems like mobile devices and video game consoles.
- Level of precision is everything when you’re dealing with colors. The more precise you are, the better quality you get. The 2nd line sets the float precision to <em mediump. You can also change it to <em lowp or <em highp if you want.
- Inputs for GLSL shaders are defined by adding a <em uniform in front of them at the top of your code. One of the most important inputs is the <em u_resolution (also called the <em iResolution) of <em type vec2 which gives you the resolution of the screen.
...
**1. Add the umbra_flutter package in your <em pubspec.yaml**
...
**3. Add the following GLSL code to the <em hello_world.glsl file**
...
The <em main() function is replaced by a <em fragment function which gives us the <em uv (normalized coordinates of the pixel) and <em fragCoord (coordinates of the pixel without <em normalization-gl_FragCoord), and returns the pixel color instead of assigning it to <em gl_FragColor.
We also added another input called <em u_time, which represents the time since we started. It’s crucial when you want to animate your Flutter shaders with respect to time.
...
**5. Create a <em my_shader.dart file somewhere in your lib folder. Add the following code within that file and import your <em HelloWorld shader in this file:**
...
What you’re doing here, in short, is compiling your shader. As the process is async, you use <em FutureBuilder to load it. Once it’s compiled, you then use the <em .shader() method on your <em UmbraShader object and pass it the required inputs.
The <em resolution is the size you want for your shader, and the <em uTime is the <em delta (time since we started). <em delta is calculated with the help of Ticker. Ticker triggers a callback for every frame that increments the <em delta by 1/60 ms.

Finally, you pass the created Flutter shader to the CustomPainter to paint.
Awesome! You just created your first shader in Flutter! Isn’t that <em exciting?
...
- It does not support <em while and <em switch statements
