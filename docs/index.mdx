...
export const CallToAction = props => {
  return <div style={{ marginTop: "150px", marginBottom: "150px", padding: "32px", border: "1px solid gray", borderRadius: "16px", backgroundImage: "url(/assets/graphics/robot_tree_farm_lighter.png)", display: "flex", flexDirection: "column", justifyContent: "center"}}>
      <h3>{ props.heading }</h3>
      { props.text }
      <InstallButton src={"/install"}>{ props.button }</InstallButton>
  </div>
};
...
<FlexRowBetween>
    <FlexColumn>
        ...
        <Accordion defaultOpen title="🧠 Smart Titles & Descriptions">
...
<CallToAction heading={"100% secure by default 🔐"} text={"All Pieces for Developers features, including our machine learning capabilities, run locally on your machine. Choose to use Pieces connected to the cloud, or not, it’s up to you!"} button={"Download on macOS"}></CallToAction>
...
<FlexRowBetween>
    ...
    <FlexColumn>
        <Card heading={"Find Code & Beyond"}
              className={"fa-brands  fa-magnifying-glass-chart"}
              text={"Global Search in Pieces for Developers not only finds you related code snippets, but also websites with more documentation about your query."}></Card>
    ...
    <FlexColumn>
        ...
        <FlexRow>
            <Card heading={"Sort by Suggested"}
                  ...
                  text={"Sort your Carousel View by Suggestions & Relevance based on recently saved Pieces. We want users to be able to reference, share, and reuse the right snippets at the right time."}></Card>
            <Card heading={"Suggested Smart Actions"}
                  className={"fa-solid fa-face-smirking"}
                  text={"Move through the UI faster and get the most out of Pieces for Developers with our Suggested Actions. Pieces will recommend different productive improvements to your workflow."}></Card>
        </FlexRow>
    </FlexColumn>
